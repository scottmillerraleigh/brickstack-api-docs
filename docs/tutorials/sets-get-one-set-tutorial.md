...
Follow these steps to `GET` an existing set from the service.

### 1. Understand the URL to the sets resource

Before you GET a set from the BrickStack API, you must understand how
to call the sets resource. The link to a specific set is:
`http://localhost:3000/set/1/` This link makes a request
for the set with the ID `1`.
...
1. Open GitBash
2. Enter the command `curl http://localhost:3000/sets/1/`
...
If you receive a response without an error, the set
...
If you receive a response without an error, details about the set
appear.
...
- The entered set ID does not exist
- Invalid syntax of the GitBash command

## Next steps

Now that you have used the `GET` command to get an existing set,
you can explore more of the API:

- Try getting all sets
