...
# Tutorial: PUT an existing set

![BrickStack Tutorial](../images/tutorial.png "BrickStack Tutorial")

Use this tutorial to use the `PUT /sets` endpoint to update an existing
LEGO set on the BrickStack API.

The `PUT` command will update a set with the entered set ID.
...
Follow these steps to PUT an existing set on the service.

### 1. Understand set format

Before you PUT a set on the BrickStack API, you must understand the
...
```shell
curl -X PUT http://localhost:3000/sets/30/ \
...
If you successfully put the set, the set
...
Now that you have used the `PUT` command to update a set,
